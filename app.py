...
def solar_resource_estimate_comparison():
...
def dnv_estimate():
    ...
    if address_option and Latitude and Longitude:
        ...
        ACCapacity = st.sidebar.text_input('Insert the AC Capacity (kWAC)',
                                           help="Project AC capacity (kWac). Default = 1")
        DCCapacity = st.sidebar.text_input('Insert the DC Capacity (kWDC)',
                                           help="Project DC capacity (kWdc). Default = ["
                                                "same as ACCapacity]")
        Mounting = st.sidebar.selectbox('What sort of racking would you like?', ('ground_fixed', 'ground_1axis'),
                                        help="Type of PV panel mounting: ground_fixed, ground_1axis")
        Azimuth = st.sidebar.text_input('Input the Azimuth',
                                        help="Azimuth angle orientation of arrays. N=0, E = 90, S = 180, W = 270 (or -90). Default = [180 for Northern hemisphere, 0 for Southern hemisphere]")
        Tilt = st.sidebar.text_input("Insert the tilt angle",
                                     help="Tilt angle of PV modules. For 1-axis trackers, use the maximum rotation angle. Default = [estimated based on selected Mounting type]")
        ModuleConfig = st.sidebar.selectbox('What Module Config would you like?', ('mod1P', 'mod2P'),
                                            help="Module layout on racking reflecting Landscape or Portrait orientation: mod1P, mod2P")
        Pitch = st.sidebar.text_input("Insert the Pitch",
                                      help="Distance between centerline axis of each row of panels (meters)")
...
page_names_to_funcs = {
    "—": intro,
    "Solar Resource Estimate Comparison": solar_resource_estimate_comparison(),
    "Dnv Solar Estimate": dnv_estimate,
    "IEC Terrain Assessment": iec_terrain_assessment
}
